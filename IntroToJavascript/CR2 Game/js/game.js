...
{
    //movement
    //change two the movement is in any direction
    if(keys.A.isDown&&this.hero.x>30){
        this.hero.x -= this.playerSpeed
        //change one, the hero grows when she walks
        this.hero.scaleX+=0.002
        this.hero.scaleY+=0.002
        this.hero.flipX = true
    }

    if(keys.S.isDown&&this.hero.y<270){
        this.hero.y += this.playerSpeed
        this.hero.scaleX+=0.002
        this.hero.scaleY+=0.002
        
    }
    if(keys.W.isDown&&this.hero.y>80){
        this.hero.y -= this.playerSpeed
        this.hero.scaleX+=0.002
        this.hero.scaleY+=0.002
    }
    if(keys.D.isDown&&this.hero.x<590){
        this.hero.x += this.playerSpeed
        this.hero.scaleX+=0.002
        this.hero.scaleY+=0.002
    ...
    // check we haven't passed max Y
        ...
        //max x
        if(this.enemytwo.x >=this.enemyMaxX){
            this.enemySpeed2 *= -1
        }
        //min x
        if(this.enemytwo.x <=this.enemyMinX){
            this.enemySpeed2 *= -1
        }
    ...
    this.enemytwo.x+=this.enemySpeed2
    this.enemythree.y+=this.enemySpeed3
    this.enemythree.x+=this.enemySpeed3
    }
    //  else if(this.enemy.y){
    //     this.enemy.y+=1
    //  }
...
//auto size growth
    //if(this.hero.scale<1){
        //this.hero.scaleX+=0.01
        //this.hero.scaleY+=0.01
    //}
